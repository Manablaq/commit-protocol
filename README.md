...
The current reviewer-hard-gates candidate adds authenticated issuer-address attestations, immutable authority/version and evidence-record/version identity, mission-version and freshness binding, expiry through the recovery deadline, distinct authenticated-issuer corroboration, and exact v2 contract/reference-model evidence-root parity. Repairable acquisition and integrity failures are now persisted as evidence-level `REPAIR_REQUIRED` responses rather than silently becoming semantic ABORTs: before the recovery deadline the principal may attach an authenticated, strictly newer successor version for the same authority, issuer, stable record ID, mission, and mission version. The original sealed evidence root remains immutable; a READY successor changes only the active evaluation root used by reevaluation. Valid negative evidence remains a semantic ABORT, while validator disagreement is not persisted as an evidence-source failure. Consequential evaluation is bound by `commit-decision-v3` to both the sealed evidence root and the exact active evaluation root. Reviewer artifacts use `commit-mission-receipt-v2` and `commit-mission-manifest-v2` and expose both roots. The frozen candidate passes 35 deterministic tests plus 46 subtests and 113 direct-runtime tests, 148 tests in the complete suite, including 18 repair-path tests, the runtime/model active-root vector, 4 deadline/allocation race tests, and 5 fee-policy tests. These candidate changes are locally certified but are not the historical v0.7 Studio Dev deployment. Any network proof for this candidate requires a fresh source-matched contract deployment; COMMIT does not claim an in-place upgrade of the historical deployed contract. External GEN delivery reconciliation and external-transfer recovery remain unproven and are not claimed by this candidate.
