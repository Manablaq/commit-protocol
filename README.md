...
The product thesis, protocol specification, accounting model, state machine, evidence model, and adversarial review are drafted. The contract requires explicit supplier authorization, rejects duplicate authority labels for the same registered origin/path, validates canonical DNS-style hosts and path prefixes, rejects boolean/out-of-range uint inputs, uses a bounded single-parent acyclic effect graph with seal-time cycle checks, accepts only the explicit `all-evidence-and-effects-v1` policy, binds every fetched record to the mission/objective/policy/intent/effect snapshot, rejects missing or malformed bodies, rejects ambiguous/non-standard JSON, and limits remote evidence bodies to 16 KiB. The last source-matched Studio Dev deployment is revision `0.5.0-authorized-graph`; the current working tree is revision `0.6.0-semantic-receipt`, which adds a full decision envelope, a single-call mission receipt, permissionless post-seal evaluation, explicit cancellation/timeout decisions, mission-scoped entitlement reads, and arithmetic overflow guards. The candidate passes GenVM lint plus the direct-runtime suite but is not yet a deployed artifact. Mission `live-mission-004` proves the previous deployed source's live path: two public evidence records were registered and fetched, semantic evaluation returned `COMMIT`, the finalized self-callback applied allocation once, and a finalized claim dispatched the beneficiary's exact entitlement. Studio Dev exposes no child transaction ID for that external message, so independent delivery reconciliation and external-transfer recovery remain unproven and are explicitly disabled. The historical Bradbury canary proved that this v0.6 runner is unavailable on Bradbury; its separate legacy probe is not evidence for the current source.
...
6. Studio Dev deployment and source/receipt verification with the matching v0.6 RC stack — complete for revision `0.5.0`; revision `0.6.0` redeployment pending
7. Native-GEN custody, finality-gated allocation, and finalized claim dispatch on Studio Dev — proven for revision `0.5.0` by `live-mission-004`; the `0.6.0` candidate must repeat source-matched live proof
