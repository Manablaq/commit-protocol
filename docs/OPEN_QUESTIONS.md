...
| ENV-03 | Exact Studio Dev, consensus and GenVM runtime? | GenVM manager v0.6.0-rc4 release archive, `5jyc...` runner, modern stdlib hash, and Python RC packages are pinned locally; no Studio Dev transaction has yet executed the current source | Deploy a minimal v0.6 canary with the matching RC CLI/SDK/Transaction Kit, then verify its full receipt |
...
| EVAL-01 | Which consensus primitive and result shape exist on Studio Dev? | The extracted v0.6 runtime exposes and locally executes `gl.vm.run_nondet` with the modern package; no Studio Dev transaction has yet executed the hardened evaluator | Confirm the same API/result shape in a successful Studio Dev evaluation transaction before enabling live custody |
...
Use Studio Dev first with the matching v0.6 RC stack: execute a minimal canary,
then deploy the hardened COMMIT source and execute evaluation, finality,
payment, and recovery canaries. Close FIN-01/PAY-01 and choose enforceable
evidence provenance. Payable custody and settlement are implemented locally
but stay blocked for live funds until network proof closes the relevant gates.
