...
| EVAL-01 | Which custom equivalence API exists in the selected v0.6 runner? | Contract-integrated evaluator and direct tests pass with `gl.vm.run_nondet`; `run_nondet_unsafe` is absent from the installed runtime | Confirm the same API/result shape on the target network before integrating evaluation with finality-gated custody |
